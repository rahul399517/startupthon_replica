...
import { createCompletedChallenge } from "@/action/auth";

const NewChallengeForm = ({ open, handleClose }) => {
  const router = useRouter();
  const [formState, action] = React.useActionState(createCompletedChallenge, {
    message: "",
    errors: {},
  });

  useEffect(() => {
    if (formState?.message === "Completed challenge created.") {
  ...
  return (
    <>
      ...
      >
          ...
          <DialogContent>
            <Box noValidate sx={{ mt: 3, width: "100%" }}>
              <TextField
                ...
                autoFocus
                placeholder="Type your challenge name"
                InputProps={{
                  sx: {
              ...
              <TextField
                ...
                error={!!formState?.errors?.founder}
        ...
        {formState?.message === "Completed challenge created." && (
          <ToastNotification message="🎉 Challenge added successfully!" />
        )}
      </Dialog>
    </>
  );
